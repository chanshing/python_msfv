...
def gen_perm2(nz, ny, nx, length, sigma, nperm, ret=False,
             verbose=True):
    """
    ...
    folder = 'perm'
    filename = ut.unparse_perm(nz, ny, nx, length, sigma, nperm)
    filepath = os.path.join(folder, filename)

    import h5py
    with h5py.File(filepath, 'w') as f:
        Ks = f.create_dataset('Ks', (nperm, nz, ny, nx, 3), float)
        ...
        Parallel(n_jobs=-1, batch_size=batch_size)\
            (delayed(_task)(i, ny, nx, corr_mtx, Ks)
...
def _task(i, ny, nx, corr_matrix, Ks):
    np.random.seed()
